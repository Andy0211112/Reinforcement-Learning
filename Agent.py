...
class ChessAgent:
    def __init__(self, name, path=None, config=Config()):
        ...
        ), lr=0.0005, momentum=0.9, weight_decay=1e-4)
        self.epoch_scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=10, gamma=0.9)
    ...
    def choose_action(self, board, max_depth=15, n_simulations=200, temperature=1.0, mode='mcts'):
        ...
        if mode == 'mcts':
            ...
            move, prob = root.best_action(temperature=temperature)
            if move is None:
                print("Warning!!!!!!!!! MCTS NOT WORKING")
                move = random.choice(list(board.legal_moves))
                prob = root.get_policy([move.uci()],[1.0])
            return move.uci(),prob
